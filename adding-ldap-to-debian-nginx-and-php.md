...
Могут возникнуть проблемы, если используется LDAP с шифрованием (SLDAP). Особенность в криптографических библиотеках, используемая в библиотеках LDAP, приводит к тому, что программы, обращающиеся к LDAP и пытающиеся изменить их эффективные привилегии, не получают соединение с сервером LDAP с помощью TLS или SSL. Это приводит к проблемам с setuid-программами в системах, использующими libnss-ldap, наподобие sudo, su или schroot и с setuid-программами, которые выполняют поиск в LDAP, наподобие sudo-ldap.
