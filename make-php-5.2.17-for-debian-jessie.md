...
```
Далее нужно изменить значение `mode`, `group` и `mode` блока `listen_options`, указывающих разрешения на чтение и запись для сокета. Веб-сервер nginx работает от имени «***www-data***» (более рание версии) или «***nginx***» (более современные версии) и создаёт одноимённую группу для своих процессов --  соотвтсвующие значения нужно указать для атрибутов **mode** и **group** конфигурационного файла FPM/ Например, следующие значения:
```xml
...
```
srw-rw---- 1 www-data www-data 0 дек 18 09:01 /home/[user]/[site]/socket/php-fpm.socket
```
